  ...
  document.addEventListener('DOMContentLoaded', async function () {
    try {
      ...
      newsData.forEach(newsItem => {
        const imageUrl = `https://be-2-section-surabaya-group-25-production.up.railway.app/public/images/${newsItem.gambar}`;
...
async function deleteNews(newsId) {
  try {
    const response = await fetch(`https://be-2-section-surabaya-group-25-production.up.railway.app/news/delete/${newsId}`, {
      method: 'DELETE',
    });
...
async function searchNews(category) {
  try {
    const response = await fetch(`https://be-2-section-surabaya-group-25-production.up.railway.app/news/getcategory//${category}`);
